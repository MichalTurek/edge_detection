...
template <typename T>
T pixelInPosition(int x, int y, std::vector<T>& vector, int width, int height) {
    if (x > width - 1)  x -= 2;
    else if (x < 0) x += 2;
...
void filter(const int x, const int y, std::vector<uint8_t>& inputImage,
  ...
  for (int i = -matrix_size / 2; i <= matrix_size / 2; i++)
  {
      ...
      {
          newPixelValue += (int16_t)((float)pixelInPosition(x + i, y + j,
              inputImage, width, height) * gaussian[iterator]);
          iterator++;
      }
  }
...
void NonMaximumSuppression(const int x, const int y, std::vector<uint8_t>& inputImage, 
    ...
    double angle = 180 * (double)pixelInPosition(x, y, inputAngle, width, height) / M_PI;
...
void edge_detection(std::vector<uint8_t>& inputImage, std::vector<uint8_t>& outputImage,
    ...
    for (auto pixel : thresholdImage) {
        Hysteresis(x, y, thresholdImage, outputImage, width, height);
        x++;
        if (x == width) {
            x = x % width;
            y++;
